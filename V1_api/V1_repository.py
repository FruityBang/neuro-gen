...
from db import db_session, ImageORM
            ...
            async with db_session() as session:
                ...
                if data.name:
                    query = sqlalchemy.orm.Query(
                        ImageORM, session=session
                        ).filter(ImageORM.name == image_dict['name'])
                    result = await session.execute(query)
                    ex_image = result.scalars().one_or_none()
                    return ex_image
                query = sqlalchemy.orm.Query(
                    ImageORM, session=session
                    ).filter(ImageORM.id == data.id)
                result = await session.execute(query)
                ex_image = result.scalars().one_or_none()
                return ex_image
    ...
    async def add_image(cls, data: ImageAdd) -> Image:
        ...
        try:
            async with db_session() as session:
                ...
                query = sqlalchemy.orm.Query(
                    ImageORM, session=session
                    ).filter(ImageORM.title == image_dict['title'])
                result = await session.execute(query)
                ex_image = result.scalars().one_or_none()
                if ex_image:
                    return ex_image
